  ...
  <c:forEach items="${placementRefsAndCollections[placement]}" var="entry">
    ...
    <c:set var="fieldDisplayName"
        ...
        <c:if test="${fn:startsWith(fieldDisplayName,'Gene Rifs')}">
          <c:set var="fieldDisplayName" value="Gene RIFs (Gene into Function)<br /><p style='font-size:12px'>Data Source: <a target='_blank' href='/${WEB_PROPERTIES['webapp.path']}/portal.do?class=DataSet&externalids=GeneRIF'>NCBI</a></p>" />	  
        </c:if>
        <!-- quick fix for MINE-769 -->
        <c:if test="${fn:startsWith(fieldDisplayName,'Pathways')}">
          <c:set var="fieldDisplayName" value="Pathways<br /><p style='font-size:12px'>Data Source: <a target='_blank' href='/${WEB_PROPERTIES['webapp.path']}/portal.do?class=DataSet&externalids=KEGG+pathways+data+set'>KEGG</a>, <a target='_blank' href='/${WEB_PROPERTIES['webapp.path']}/portal.do?class=DataSet&externalids=Reactome+data+set'>Reactome</a></p>" />	  
        </c:if>
