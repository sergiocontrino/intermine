 ...
 * @author julie
 */
public class PlantOntologyDisplayer extends ReportDisplayer
{
    /**
     ...
     */
    public PlantOntologyDisplayer(ReportDisplayerConfig config, InterMineAPI im) {
        super(config, im);
    }
    ...
    private static PathQuery buildQuery(Model model, Integer geneId) {
        ...
        q.addConstraint(Constraints.isNull("Gene.poAnnotation.qualifier"));
    ...
    private boolean isGoLoadedForOrganism(String organismField, Profile profile) {
        if (!organismCache.containsKey(organismField)) {
            ...
            q.addViews("Gene.poAnnotation.ontologyTerm.name");
